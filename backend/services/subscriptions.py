...
class SubscriptionsService:
    ...
    async def bulk_create(self, items: List[Dict[str, Any]], user_id: Optional[str] = None) -> List[Subscriptions]:
        """Bulk-create multiple subscriptions in a single transaction (avoids N+1 commits)"""
        try:
            objs = []
            for data in items:
                if user_id:
