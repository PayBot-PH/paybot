...
async def telegram_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        async def get_tg_wallet(uid: str) -> Wallets:
            res = await db.execute(select(Wallets).where(Wallets.user_id == uid))
            w = res.scalar_one_or_none()
            if not w:
                now_w = datetime.now()
                w = Wallets(user_id=uid, balance=0.0, currency="PHP", created_at=now_w, updated_at=now_w)
                db.add(w)
                await db.commit()
                await db.refresh(w)
            return w
        ...
        # ==================== /invoice ====================
            ...
            if len(parts) < 2:
                await telegram_service.send_message(chat_id, "❌ Usage: /invoice [amount] [description]\nExample: /invoice 500 Monthly subscription")
            else:
                try:
                    ...
                    if result.get("success"):
                        reply = (f"✅ <b>Invoice Created!</b>\n\n💰 Amount: ₱{amount:,.2f}\n📝 {description}\n"
                                 f"🔗 Pay: {result.get('invoice_url', '')}\n🆔 <code>{result.get('external_id', '')}</code>")
                        now = datetime.now()
                        txn = Transactions(user_id="telegram", transaction_type="invoice",
        ...
        # ==================== /qr ====================
            ...
            parts = text.split(maxsplit=2)
            if len(parts) < 2:
                await telegram_service.send_message(chat_id, "❌ Usage: /qr [amount] [description]")
            else:
                try:
                    amount = float(parts[1])
                    ...
                    if result.get("success"):
                        reply = (f"✅ <b>QR Code Created!</b>\n\n💰 ₱{amount:,.2f}\n📱 QR: <code>{result.get('qr_string', '')}</code>\n"
                                 f"🆔 <code>{result.get('external_id', '')}</code>")
                        now = datetime.now()
                        txn = Transactions(user_id="telegram", transaction_type="qr_code",
        ...
        # ==================== /link ====================
            ...
            parts = text.split(maxsplit=2)
            if len(parts) < 2:
                await telegram_service.send_message(chat_id, "❌ Usage: /link [amount] [description]")
            else:
                try:
                    amount = float(parts[1])
                    ...
                    if result.get("success"):
                        reply = (f"✅ <b>Payment Link Created!</b>\n\n💰 ₱{amount:,.2f}\n📝 {description}\n"
                                 f"🔗 {result.get('payment_link_url', '')}\n🆔 <code>{result.get('external_id', '')}</code>")
                        now = datetime.now()
                        txn = Transactions(user_id="telegram", transaction_type="payment_link",
        ...
        # ==================== /va ====================
            ...
            parts = text.split(maxsplit=2)
            if len(parts) < 3:
                await telegram_service.send_message(chat_id, "❌ Usage: /va [amount] [bank_code]\nExample: /va 1000 BDO\nBanks: BDO, BPI, UNIONBANK, RCBC")
            else:
                try:
                    amount = float(parts[1])
                    ...
                    if result.get("success"):
                        reply = (f"✅ <b>Virtual Account Created!</b>\n\n🏦 Bank: {bank_code}\n💰 ₱{amount:,.2f}\n"
                                 f"🔢 Account: <code>{result.get('account_number', '')}</code>\n🆔 <code>{result.get('external_id', '')}</code>")
                        now = datetime.now()
                        txn = Transactions(user_id="telegram", transaction_type="virtual_account",
        ...
        # ==================== /ewallet ====================
            ...
            parts = text.split(maxsplit=2)
            if len(parts) < 3:
                await telegram_service.send_message(chat_id, "❌ Usage: /ewallet [amount] [provider]\nProviders: GCASH, GRABPAY, PAYMAYA")
            else:
                try:
                    amount = float(parts[1])
                    provider = parts[2].upper()
                    channel_map = {"GCASH": "PH_GCASH", "GRABPAY": "PH_GRABPAY", "PAYMAYA": "PH_PAYMAYA",
                    ...
                    if result.get("success"):
                        ...
                        reply = (f"✅ <b>E-Wallet Charge Created!</b>\n\n📱 {provider}\n💰 ₱{amount:,.2f}\n"
                                 f"{'🔗 Pay: ' + checkout if checkout else ''}\n🆔 <code>{result.get('external_id', '')}</code>")
                        now = datetime.now()
                        txn = Transactions(user_id="telegram", transaction_type="ewallet",
        ...
        # ==================== /disburse ====================
            ...
            if len(parts) < 4:
                await telegram_service.send_message(chat_id, "❌ Usage: /disburse [amount] [bank_code] [account_number]\nExample: /disburse 500 BDO 1234567890")
            else:
                try:
                    amount = float(parts[1])
                    bank_code = parts[2].upper()
                    ...
                    if result.get("success"):
                        reply = (f"✅ <b>Disbursement Created!</b>\n\n💸 ₱{amount:,.2f}\n🏦 {bank_code}\n"
                                 f"🔢 {account_number}\n🆔 <code>{result.get('external_id', '')}</code>")
                        now = datetime.now()
                        disb = Disbursements(user_id="telegram", external_id=result.get("external_id", ""),
                            ...
                            description=f"TG disbursement", status="pending", disbursement_type="single",
                            created_at=now, updated_at=now)
                        db.add(disb)
                        await db.commit()
                    else:
        ...
        # ==================== /refund ====================
            ...
            parts = text.split(maxsplit=2)
            if len(parts) < 2:
                await telegram_service.send_message(chat_id, "❌ Usage: /refund [external_id] [amount]\nExample: /refund inv-abc123 500")
            else:
                ...
                if not txn:
                    ...
                    if refund_amount > txn.amount:
                        ...
                        ref = Refunds(user_id="telegram", transaction_id=txn.id,
        ...
        # ==================== /status ====================
            ...
            if len(parts) < 2:
                await telegram_service.send_message(chat_id, "❌ Usage: /status [external_id]")
            else:
                ext_id = parts[1].strip()
                result = await db.execute(select(Transactions).where(Transactions.external_id == ext_id))
                txn = result.scalar_one_or_none()
                if txn:
                    ...
                    reply = (f"📊 <b>Transaction Status</b>\n\n🆔 <code>{txn.external_id}</code>\n"
        ...
        # ==================== /balance ====================
            ...
            wallet = await get_tg_wallet(tg_user_id)
            reply = (f"💰 <b>Wallet Balance</b>\n\n💵 <b>₱{wallet.balance:,.2f}</b>\n💱 {wallet.currency or 'PHP'}\n\n"
        ...
        # ==================== /send ====================
            ...
            parts = text.split(maxsplit=2)
            if len(parts) < 3:
                await telegram_service.send_message(chat_id, "❌ Usage: /send [amount] [recipient]")
            else:
                try:
                    amount = float(parts[1])
                    recipient = parts[2]
                    if amount <= 0:
                        ...
                        if wallet.balance < amount:
                            ...
                            wtxn = Wallet_transactions(user_id=tg_user_id, wallet_id=wallet.id,
                                transaction_type="send", amount=amount, balance_before=bb,
                                balance_after=wallet.balance, recipient=recipient,
                                note=f"Sent to {recipient} via Telegram", status="completed",
                                reference_id=f"tg-send-{wallet.id}-{int(now.timestamp())}", created_at=now)
                            db.add(wtxn)
                            await db.commit()
                            payment_event_bus.publish({"event_type": "wallet_update", "user_id": tg_user_id,
                                "wallet_id": wallet.id, "balance": wallet.balance, "transaction_type": "send",
                                "amount": amount, "transaction_id": wtxn.id})
                            reply = f"✅ <b>Sent!</b>\n\n💸 ₱{amount:,.2f} → {recipient}\n💰 Balance: <b>₱{wallet.balance:,.2f}</b>"
                            await telegram_service.send_message(chat_id, reply)
                except ValueError:
                    await telegram_service.send_message(chat_id, "❌ Invalid amount.")
        ...
        # ==================== /withdraw ====================
            ...
            parts = text.split(maxsplit=1)
            if len(parts) < 2:
                await telegram_service.send_message(chat_id, "❌ Usage: /withdraw [amount]")
            else:
                try:
                    amount = float(parts[1])
                    if amount <= 0:
                        ...
                        if wallet.balance < amount:
                            ...
                            wtxn = Wallet_transactions(user_id=tg_user_id, wallet_id=wallet.id,
                                ...
                                balance_after=wallet.balance, note="Withdrawal via Telegram", status="completed",
                                reference_id=f"tg-withdraw-{wallet.id}-{int(now.timestamp())}", created_at=now)
                            db.add(wtxn)
                            await db.commit()
                            payment_event_bus.publish({"event_type": "wallet_update", "user_id": tg_user_id,
                                "wallet_id": wallet.id, "balance": wallet.balance, "transaction_type": "withdraw",
                                "amount": amount, "transaction_id": wtxn.id})
        ...
        # ==================== /report ====================
            ...
            paid_r = await db.execute(select(func.coalesce(func.sum(Transactions.amount), 0)).where(
                Transactions.status == "paid", Transactions.created_at >= start))
            paid = float(paid_r.scalar() or 0)
            total_r = await db.execute(select(func.count(Transactions.id)).where(Transactions.created_at >= start))
            total = total_r.scalar() or 0
            paid_c = await db.execute(select(func.count(Transactions.id)).where(
                Transactions.status == "paid", Transactions.created_at >= start))
            paid_count = paid_c.scalar() or 0
            rate = round((paid_count / total * 100) if total > 0 else 0, 1)
            reply = (f"📊 <b>{period.title()} Report</b>\n\n"
        ...
        # ==================== /fees ====================
            ...
            parts = text.split(maxsplit=2)
            if len(parts) < 3:
                await telegram_service.send_message(chat_id, "❌ Usage: /fees [amount] [method]\nMethods: invoice, qr_code, ewallet, virtual_account, card, disbursement")
            else:
                try:
                    amount = float(parts[1])
        ...
        # ==================== /subscribe ====================
            ...
            if len(parts) < 3:
                ...
                try:
                    ...
                    now = datetime.now()
                    from datetime import timedelta
                    sub = Subscriptions(user_id="telegram", plan_name=plan_name, amount=amount,
                        currency="PHP", interval="monthly", customer_name=username,
                        status="active", next_billing_date=now + timedelta(days=30),
                        total_cycles=0, external_id=f"sub-tg-{__import__('uuid').uuid4().hex[:8]}",
                        created_at=now, updated_at=now)
                    db.add(sub)
                    await db.commit()
                    reply = f"✅ <b>Subscription Created!</b>\n\n📋 {plan_name}\n💰 ₱{amount:,.2f}/month\n📅 Next billing: {(now + timedelta(days=30)).strftime('%Y-%m-%d')}"
                    await telegram_service.send_message(chat_id, reply)
                except ValueError:
                    await telegram_service.send_message(chat_id, "❌ Invalid amount.")
        ...
        # ==================== /remind ====================
            ...
            parts = text.split(maxsplit=1)
            if len(parts) < 2:
                await telegram_service.send_message(chat_id, "❌ Usage: /remind [external_id]")
            else:
                ext_id = parts[1].strip()
                result = await db.execute(select(Transactions).where(Transactions.external_id == ext_id))
                txn = result.scalar_one_or_none()
                if txn and txn.status == "pending":
                    ...
                    if txn.telegram_chat_id:
                        await telegram_service.send_message(txn.telegram_chat_id, msg)
        ...
        # ==================== /help ====================
            ...
            await telegram_service.send_message(chat_id, "🤖 Unknown command. Type /help for all commands.")

        # Log the interaction
        log = Bot_logs(user_id="telegram", log_type="command", message=text,
            telegram_chat_id=chat_id, telegram_username=username,
            command=text.split()[0] if text else "", created_at=datetime.now())
        db.add(log)
        await db.commit()
...
async def get_bot_info(current_user: UserResponse = Depends(get_current_user)):
    try:
        service = TelegramService()
