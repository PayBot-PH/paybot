...
def _make_github_sig(secret: str, body: bytes) -> str:
    """Compute the X-Hub-Signature-256 header value for a given payload."""
    import hashlib
...
def _pr_payload(number: int = 1, action: str = "opened", state: str = "open") -> dict:
    return {
        "action": action,
        "pull_request": {
            "number": number,
            ...
            "state": state,
            "html_url": f"https://github.com/csphi/paybot/pull/{number}",
            "draft": False,
            "merged": action == "closed" and state == "closed",
...
class TestGitHubWebhook:
    """Tests for the GitHub webhook and PR listing endpoints."""

    def test_webhook_ping_returns_no_content(self, client):
        """GitHub sends a ping event on webhook creation — should be acknowledged."""
        import json as _json
        body = _json.dumps({"hook_id": 1}).encode()
        r = client.post(
    ...
    def test_webhook_non_pr_event_ignored(self, client):
        """Non-pull_request events (e.g. push) must be silently ignored."""
        import json as _json
    ...
    def test_webhook_pull_request_event_stored(self, client):
        """A valid pull_request event is stored and appears in the PR list."""
        import json as _json
        from services.github_service import github_pr_store

        payload = _pr_payload(number=9001, action="opened")
        ...
        pr = github_pr_store.get_by_number(9001)
        assert pr is not None
        assert pr["title"] == "Test PR #9001"
        assert pr["state"] == "open"
        assert pr["action"] == "opened"

    def test_webhook_signature_verified_with_secret(self, client):
        """When a secret is set, an invalid signature must be rejected with 403."""
        import json as _json
        from unittest.mock import patch
        from core.config import Settings

        payload = _pr_payload(number=9002)
        body = _json.dumps(payload).encode()

        patched = Settings()
        patched.github_webhook_secret = "supersecret"

        with patch("routers.github.settings", patched):
            # Bad signature
            r = client.post(
                "/api/v1/github/webhook",
                content=body,
    ...
    def test_webhook_valid_signature_accepted(self, client):
        """A correctly signed payload must be accepted even when a secret is set."""
        import json as _json
        from unittest.mock import patch
        from core.config import Settings

        secret = "my_webhook_secret"
        payload = _pr_payload(number=9003)
        body = _json.dumps(payload).encode()
        sig = _make_github_sig(secret, body)

        patched = Settings()
        patched.github_webhook_secret = secret
    ...
    def test_pull_requests_listing_requires_auth(self, client):
        """GET /api/v1/github/pull-requests must require authentication."""
        r = client.get("/api/v1/github/pull-requests")
        assert r.status_code == 401

    def test_pull_requests_listing_authenticated(self, client, auth_headers):
        """Authenticated users can retrieve the PR list."""
        r = client.get("/api/v1/github/pull-requests", headers=auth_headers)
        assert r.status_code == 200
        data = r.json()
    ...
    def test_pull_requests_sorted_by_number_desc(self, client, auth_headers):
        """PRs returned from the listing endpoint are ordered newest-first."""
        import json as _json
        from services.github_service import github_pr_store

        # Ingest two PRs with known numbers
        for num in (5, 10):
            payload = _pr_payload(number=num)
            body = _json.dumps(payload).encode()
            client.post(
                "/api/v1/github/webhook",
                content=body,
        ...
        items = r.json()["items"]
        if len(items) >= 2:
            numbers = [p["number"] for p in items]
